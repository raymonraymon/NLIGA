%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%
%  Elastic analysis of Pichched Cylinder shell based on Reissner-Mindlin 
%  shell assumption;
%  The model is built with a singular NURBS surface.
...
R = 300;
L = 600;
geo = geo_pinched_cylinder_shell(R, L);
...
forceNode   = mesh.nCptsU*(mesh.nCptsV-1)+1;
...
dbc = [dbc; bottomNodes,     5*ones(size(bottomNodes)),   zeros(size(bottomNodes))];
 % xz plane symmetric, uy = theta_x = theta_z = 0 
...
dbc = [dbc; topNodes,        6*ones(size(topNodes)),      zeros(size(topNodes))];
 % yz plane symmetric, ux = theta_y = theta_z = 0
...
dbc = [dbc; leftNodes,       6*ones(size(leftNodes)),     zeros(size(leftNodes))];
 % xy plane symmetric, uz = theta_x = theta_y = 0
...
tanx = zeros(mesh.nCpts,3);
tany = zeros(mesh.nCpts,3);
for i = 1:size(ga,1)
    gauPts = ga(i,:);
    e      = elem_index(i);
    ...
    tanx(i,:) = dxdxi / norm(dxdxi);
    tany(i,:) = cross(n,tanx(i,:));
end
