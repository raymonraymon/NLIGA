...
%  The model is built with three conforming NURBS patches.
%
%  ---------------------------------------
...
mesh{1,3}.nodeNum(1,:) = mesh{1,1}.nodeNum(end,:);
mesh{1,3}.globCoords = zeros(mesh{1,3}.nCptsV*(mesh{1,3}.nCptsU-1),4);
for j = 2:mesh{1,3}.nCptsV
    for i = 1:mesh{1,3}.nCptsU
        aa = (j-1)*(mesh{1,2}.nCptsU)+i;
        bb = (j-2)*(mesh{1,2}.nCptsU)+i;
        mesh{1,3}.nodeNum(j,i) = bb + mesh{1,2}.nodeNum(end,end);
        mesh{1,3}.globCoords(bb,:) = mesh{1,3}.coords(aa,:); 
    end
end
...
bcNodes1 = mesh{1,3}.nodeNum(:,1);
bcNodes2 = mesh{1,1}.nodeNum(:,1);
bcNodes3 = mesh{1,1}.nodeNum(1,:)';
bcNodes4 = mesh{1,2}.nodeNum(1,:)';
bcNodes5 = mesh{1,2}.nodeNum(:,end);
bcNodes6 = mesh{1,2}.nodeNum(end,:)';
bcNodes7 = mesh{1,3}.nodeNum(:,end);
bcNodes8 = mesh{1,3}.nodeNum(end,:)';
bcNodes = unique([bcNodes1; bcNodes2; bcNodes3; bcNodes4; bcNodes5; bcNodes6; bcNodes7; bcNodes8]);
...
for kk = 1:length(mesh)
    ...
    vmesh{1,kk}.vertices = zeros(numpts,3);
    ...
    for i = 1:size(kntcrv{1,kk}.linpts,1)
        ...
        x = vmesh{1,kk}.vertices(count,1);
        y = vmesh{1,kk}.vertices(count,2);
        ux = sin(x*pi)*sin(y*pi);
        vmesh{1,kk}.exactdisplacement(count) = ux;
    end
end
